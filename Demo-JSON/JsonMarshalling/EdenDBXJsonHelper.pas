...
{$IFEND}
  ...
  public
    ...
    class function ObjToJSON(const AObj: TObject): TJSONObject; overload; static;
    class function ObjToJSON(const ATypeInfo, AInstance: Pointer): TJSONObject; overload; static;
    class procedure JsonToObj(const AJObj: TJSONObject; const AObj: TObject); overload; static;
    class procedure JsonToObj(const AJObj: TJSONObject; const ATypeInfo, AInstance: Pointer); overload; static;
  end;
...
class procedure TDBXJSONToolsHelper.JsonToObj(const AJObj: TJSONObject;
  const ATypeInfo, AInstance: Pointer);
var
  ...
  LJPair: TJSONPair;
begin
  for LField in LContext.GetType(ATypeInfo).GetFields do
  begin
    LJPair := AJObj.Get(LField.Name);
    if Assigned(LJPair) then
    begin
      LFieldValue := LField.GetValue(AInstance);
      if LFieldValue.Kind in [tkInteger, tkInt64] then
        LFieldValue := LFieldValue.From(LJPair.JsonValue.AsJsonNumber.AsInt)
      else if LFieldValue.Kind in [tkFloat] then
        LFieldValue := LFieldValue.From(LJPair.JsonValue.AsJsonNumber.AsDouble)
      else if LFieldValue.Kind = tkEnumeration then
      begin
        if LJPair.JsonValue.IsJsonTrue then
          LFieldValue := LFieldValue.From(True)
        else
          LFieldValue := LFieldValue.From(False);
      end
      else
        LFieldValue := LFieldValue.From(LJPair.JsonValue.Value);

      LField.SetValue(AInstance, LFieldValue);
    end;
  end;
end;
...
class procedure TDBXJSONToolsHelper.JsonToObj(const AJObj: TJSONObject;
  const AObj: TObject);
  procedure JSONToRttiRecord(const ARttiRecordType: TRttiRecordType; const AParentValue: TValue; const AJsonObject: TJSONObject);
  var
    LRecordField: TRttiField;
    LRecordFieldValue: TValue;
    LSubRecord: TRttiRecordType;
    LJPair: TJSONPair;
  begin
    for LRecordField in ARttiRecordType.GetFields do
    begin
      LJPair := AJsonObject.Get(LRecordField.Name);
      if not Assigned(LJPair) then
        Continue;
      LRecordFieldValue := LRecordField.GetValue(AParentValue.GetReferenceToRawData);
      if LRecordFieldValue.Kind in [tkInteger, tkInt64] then
        LRecordFieldValue := LRecordFieldValue.From(LJPair.JsonValue.AsJsonNumber.AsInt)
      else if LRecordFieldValue.Kind in [tkFloat] then
        LRecordFieldValue := LRecordFieldValue.From(LJPair.JsonValue.AsJsonNumber.AsDouble)
      else if LRecordFieldValue.Kind = tkEnumeration then
      begin
        if LJPair.JsonValue.IsJsonTrue then
          LRecordFieldValue := LRecordFieldValue.From(True)
        else
          LRecordFieldValue := LRecordFieldValue.From(False);
      end
      else if LRecordFieldValue.Kind = tkRecord then
      begin
        LSubRecord := LRecordField.FieldType.AsRecord;
        JSONToRttiRecord(LSubRecord, LRecordFieldValue, LJPair.JsonValue.AsJsonObject);
      end
      else
        LRecordFieldValue := LRecordFieldValue.From(LJPair.JsonValue.Value);

      LRecordField.SetValue(AParentValue.GetReferenceToRawData, LRecordFieldValue);
    end;
  end;
var
  LContext: TRttiContext;
  ...
  LSubRecord: TRttiRecordType;
begin
  for LField in LContext.GetType(AObj.ClassInfo).GetFields do
  begin
    LJPair := AJObj.Get(LField.Name);
    if Assigned(LJPair) then
    begin
      LFieldValue := LField.GetValue(AObj);
      if LFieldValue.Kind in [tkInteger, tkInt64] then
        LFieldValue := LFieldValue.From(LJPair.JsonValue.AsJsonNumber.AsInt)
      else if LFieldValue.Kind in [tkFloat] then
        LFieldValue := LFieldValue.From(LJPair.JsonValue.AsJsonNumber.AsDouble)
      ...
      else if LFieldValue.Kind = tkRecord then
      begin
        LSubRecord := LField.FieldType.AsRecord;
        JSONToRttiRecord(LSubRecord, LFieldValue, LJPair.JsonValue.AsJsonObject);
      end
      else
        LFieldValue := LFieldValue.From(LJPair.JsonValue.Value);

      LField.SetValue(AObj, LFieldValue);
    end;
  end;
end;

class function TDBXJSONToolsHelper.ObjToJSON(const AObj: TObject): TJSONObject;
  function RttiRecordToJson(const ARttiRecordType: TRttiRecordType; const AParentValue: TValue): TJSONObject;
  var
    LRecordField: TRttiField;
    LRecordFieldValue: TValue;
    LSubRecord: TRttiRecordType;
  begin
    ...
    begin
      LRecordFieldValue := LRecordField.GetValue(AParentValue.GetReferenceToRawData);
      if LRecordFieldValue.Kind in [tkInteger, tkInt64, tkFloat] then
        Result.AddPair(LRecordField.Name, TJSONNumber.Create(LRecordFieldValue.AsExtended))
      else if LRecordFieldValue.Kind = tkEnumeration then
      begin
        if LRecordFieldValue.AsBoolean then
          Result.AddPair(LRecordField.Name, TJSONTrue.Create)
        else
          Result.AddPair(LRecordField.Name, TJSONFalse.Create);
      end
      else if LRecordFieldValue.Kind = tkRecord then
      begin
        LSubRecord := LRecordField.FieldType.AsRecord;
        Result.AddPair(LRecordField.Name, RttiRecordToJson(LSubRecord, LRecordFieldValue))
      end
      else
        Result.AddPair(LRecordField.Name, LRecordFieldValue.AsString);
    end;
  end;
var
  LContext: TRttiContext;
...
begin
  ...
  begin
    LFieldValue := LField.GetValue(AObj);
    if LFieldValue.Kind in [tkInteger, tkInt64, tkFloat] then
      Result.AddPair(LField.Name, TJSONNumber.Create(LFieldValue.AsExtended))
    else if LFieldValue.Kind = tkEnumeration then
    begin
      if LFieldValue.AsBoolean then
        Result.AddPair(LField.Name, TJSONTrue.Create)
      else
        Result.AddPair(LField.Name, TJSONFalse.Create);
    end
    else if LFieldValue.Kind = tkRecord then
    begin
      LRecord := LField.FieldType.AsRecord;
      Result.AddPair(LField.Name, RttiRecordToJson(LRecord, LFieldValue))
    end
    else
      Result.AddPair(LField.Name, LFieldValue.AsString);
  end;
end;

class function TDBXJSONToolsHelper.ObjToJSON(const ATypeInfo, AInstance: Pointer): TJSONObject;
var
  LContext: TRttiContext;
  LField: TRttiField;
  LFieldValue: TValue;
